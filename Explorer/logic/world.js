...
function generateOre_Iron(x,y){
  let size =1+Math.floor(rand(x));
  function determineDepth() {
    depth = 8 + Math.round(15 * Math.random())
    return(depth)
  }

  for(let i=0;i<size;i++) if(y-i>0) world[(y-i)+determineDepth()][x]=TILE_IRON;
}
...
function generate(){
  ...
  for(let x=0;x<cols;x++){
    ...
    if(rand(x*7)>.82) growTree(x,height);

    if(rand(x*7>0.6)) generateOre_Iron(x,height);
